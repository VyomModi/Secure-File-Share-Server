...
// Store file in memory (no disk — Render's disk is ephemeral)
const upload = multer({
  storage: multer.memoryStorage(),
  limits: { fileSize: 50 * 1024 * 1024 }, // 50 MB
});

// No-op for compatibility (no uploads dir needed anymore)
...
router.get("/", async (req: AuthRequest, res) => {
  try {
    ...
    const files = await FileModel.find({ ownerUserId: req.user.id })
      .select("-encryptedData") // don't send file bytes in list
      .sort({ uploadDate: -1 })
      .lean();
...
router.post("/upload", upload.single("file"), async (req: AuthRequest, res) => {
  try {
    ...
    // Hash original content for integrity check
    const sha256Hash = crypto
      .createHash("sha256")
      .update(file.buffer)
      .digest("hex");

    // Encrypt and store bytes directly in MongoDB
    ...
    const { encryptedData: _, ...safeDoc } = docObj as any;
    res.status(201).json({ ...safeDoc, id: String(docObj._id) });
  } catch (err) {
